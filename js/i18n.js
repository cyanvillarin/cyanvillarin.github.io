...
const translations = {
    en: {
        ...
        "workflow.pde.title": "Working as a Product Engineer (PdE)",
        "workflow.pde.body": `At work I currently take on a <strong>Product Engineer (PdE)</strong> role — a hybrid of project manager and engineer, where one person carries a feature from definition all the way to release instead of handing it off between roles. In practice that means writing and refining the <strong>product spec</strong>, keeping the <strong>product spec checklist</strong> up to date as decisions change, <strong>implementing</strong> the feature myself, <strong>coordinating with QA</strong> on what needs to be verified and how, and driving the <strong>release checklist</strong> before ship.`,
        "workflow.pde.pilot": `This setup is still a <strong>pilot</strong> on the <strong>Mercoin</strong> client team, and I was the <strong>second person</strong> to try it. What makes it interesting is how much it changes the shape of the work: because the same person owns both the spec and the code, ambiguity gets resolved while it is still a question rather than surfacing later as a bug, and the release checklist becomes a real quality gate instead of a formality. The most valuable part has been learning the process end to end — the checkpoints, the review points, and who signs off on what — since the entire point of it is to make sure nothing ships with bugs.`,
    ...
    ja: {
        ...
        "workflow.pde.title": "プロダクトエンジニア（PdE）としての働き方",
        "workflow.pde.body": `現在、業務では<strong>プロダクトエンジニア（PdE）</strong>の役割を担っています。これはプロジェクトマネージャーとエンジニアを兼ねた役割で、機能を役割ごとに引き継いでいくのではなく、一人が定義からリリースまでを一貫して担当します。具体的には、<strong>プロダクト仕様</strong>の作成と改善、意思決定の変化に合わせた<strong>プロダクト仕様チェックリスト</strong>の更新、機能の<strong>実装</strong>、検証内容と検証方法についての<strong>QAとの調整</strong>、そしてリリース前の<strong>リリースチェックリスト</strong>の推進です。`,
        "workflow.pde.pilot": `この体制は<strong>Mercoin</strong>クライアントチームでまだ<strong>パイロット段階</strong>にあり、私はこれを試した<strong>2人目</strong>のメンバーです。興味深いのは、これによって仕事の形が大きく変わることです。仕様とコードを同じ人が持つため、曖昧な点が後からバグとして表面化するのではなく、まだ疑問の段階で解消されます。またリリースチェックリストも形式的なものではなく、実質的な品質ゲートとして機能します。最も価値があったのは、チェックポイント、レビューポイント、誰が何を承認するのかといったプロセスをエンドツーエンドで学べたことです。このプロセス全体の目的は、バグをリリースしないことにあるからです。`,
